...
def tambah_user():
    ...
    while True:
        ...
        # ambil semua password dari database
        semua_password = [u["password"] for u in db.data.get("users", {}).values()]

        # cek apakah password sudah dipakai
        if password in semua_password:
            print(f"{Theme.FAIL}❌ Password sudah digunakan, gunakan yang lain!{Theme.ENDC}")
            continue
...
def tambah_gudang():
    ...
    db.data["gudang"][id_g] = {"nama": nama, "stok": {}}
...
def tambah_toko():
    ...
    db.data["toko"][id_t] = {"nama": nama, "stok": {}}
...
def distribusi():
    ...
    kategori = db.data["barang"].get(id_b, {}).get("kategori", "-")
    ...
    try:
        ...
        # 4. Jumlah tidak boleh <= 0
        if jumlah <= 0:
            print("❌ Jumlah harus lebih dari 0!")
            input("Enter...")
            return

        # 🔥 5. VALIDASI PENTING (INI YANG KAMU MAU)
